...
InitGlui( void )
{
	...
	spinner = new GLUI_Spinner(Glui, "mid value:", &white_range);
	...
	for (i=0; i<this_poly->ntris; i++) {
		...
		Triangle *temp_t=this_poly->tlist[i];
			...
			for (j=0; j<3; j++) {
				...
				switch (WhichColor) {
				case RAINBOW:
					rainbow_color(s, s_min, s_max, rgb);
					break;
				case BWR:
					bwr(s, s_min, s_max, rgb);
					break;
				case HEATMAP:
					heatmap(s, s_min, s_max, rgb);
					break;
				case DISCRETE:
					discrete(s, s_min, s_max, rgb);
					break;
				case NONLINEAR:
					nonlinear(s,s_min,s_max,rgb);
					break;
				case WHITE:
					rgb[0] = rgb[1] = rgb[2] = 1;
					break;
