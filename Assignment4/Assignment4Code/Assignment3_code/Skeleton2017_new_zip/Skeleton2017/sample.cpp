...
int x_value = 25, y_value = 25, z_value = 25;
int contours = 1;
float rgb[3] = {0.,1.,0};
// the color numbers:
...
int XYplane =1 , YZplane=1 , ZXplane =1 ,wireframe = 0;
...
InitGlui( void )
{
	...
	Glui->add_checkbox("XY", &XYplane);
	Glui->add_checkbox("YZ", &YZplane);
	Glui->add_checkbox("ZX", &ZXplane);
	Glui->add_checkbox("wireframe", &wireframe);

	spinner = new GLUI_Spinner(Glui, "z min value:", &z_min);
	spinner->set_int_limits(-1, 1);
