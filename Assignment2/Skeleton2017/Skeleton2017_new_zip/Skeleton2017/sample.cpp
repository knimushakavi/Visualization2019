...
const char *WINDOWTITLE = { "OpenGL / GLUT / GLUI Sample -- Kusuma Nimushakavi" };
...
InitGlui( void )
{
	...
	//Glui->add_spinner("MidValue for BWR",&);
	...
	panel = Glui->add_panel(  "Choose object to open " );
			...
			obj_list->add_item (5, "torus_field");
...
void heatmap(float s, float s_max, float s_min, float rgb[3]) {
	...
	//float hsv[3] = { 1. };
	float scale = (s_max - s_min) / 4;
	float s_one = s_min + scale;
	float s_two = s_one + scale;

	if (t <= s_one) {
		rgb[0] = ((s - s_min) / (s_one - s_min));
		rgb[1] = 0 ;
		rgb[2] = 0.;
	}
	else if(t > s_one && s <= s_two) {
		rgb[0] = 1;
		rgb[1] = ((s - s_one) / (s_two - s_one));
		rgb[2] = 0;
	}
	else if (s > s_two && s  <= s_max) {
		rgb[0] = 1;
		rgb[1] = 1;
		rgb[2] = ((s - s_two) / (s_max - s_two));
	}
	...
	for (i=0; i<this_poly->ntris; i++) {
		...
		for (j = 0; j < 3; j++) {
			...
			for (j=0; j<3; j++) {
				...
				switch(WhichColor){
					case RAINBOW:
						rainbow_color(scalar, smin, smax, rgb);
						break;
					case BWR:
						 bwr(scalar, smin, smax, rgb);		
						break;
					case HEATMAP:
						heatmap(scalar, smin, smax, rgb);	
						break;
					case DISCRETE:
						discrete(scalar, smin, smax, rgb);
						break;
					case NONLINEAR:
						//printf("NONLINEAR");
						break;
